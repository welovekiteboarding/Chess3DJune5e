defmodule Mix.Tasks.Symphony.Operate do
  ...
  alias Symphony1.Planning.{ControlBranchGuard, Graph, Status}
  ...
  @doc false
  def run_with_opts(args, runtime_opts) do
    ...
    tick_fn =
      runtime_opt(runtime_opts, :tick_fn, :operate_tick_fn, &run_tick/4)

    should_continue =
      runtime_opt(
        runtime_opts,
        :should_continue,
        :operate_should_continue,
        &default_should_continue/0
      )

    sleep_fn =
      runtime_opt(runtime_opts, :sleep_fn, :operate_sleep_fn, &Process.sleep/1)
  ...
  def run_tick(graph_path, team_key, cwd, opts) do
    ...
    cycle_id = new_cycle_id(runtime_opts)
    ...
    case StaleGraphGuard.check(cwd, graph_path, graph) do
      ...
      :ok ->
        findings =
          if candidates != [] do
            compute_findings(candidates, team_key, runtime_opts)
          else
            %{}
          end
        ...
        if stale_findings != %{} do
          ...
          linear_config = build_linear_config(team_key, runtime_opts)
          ...
          issue_fetcher =
            runtime_opt(runtime_opts, :plan_sync_issue_fetcher, :plan_sync_issue_fetcher)
  ...
  defp build_linear_config(team_key, runtime_opts) do
    case RuntimeConfig.linear_config(team_key, runtime_opts) do
      {:ok, config} ->
        config
  ...
  defp compute_findings(candidates, team_key, runtime_opts) do
    linear_config = build_linear_config(team_key, runtime_opts)

    issue_fetcher = runtime_opt(runtime_opts, :operate_issue_fetcher, :operate_issue_fetcher)
  ...
  defp new_cycle_id(runtime_opts) do
    generator =
      runtime_opt(
        runtime_opts,
        :observability_cycle_id_generator,
        :observability_cycle_id_generator,
        &Recorder.new_cycle_id/0
      )

    generator.()
  end

  defp runtime_opt(runtime_opts, runtime_key, env_key) do
    Keyword.get(runtime_opts, runtime_key, Application.get_env(:symphony_1, env_key))
  end

  defp runtime_opt(runtime_opts, runtime_key, env_key, default) do
    Keyword.get(runtime_opts, runtime_key, Application.get_env(:symphony_1, env_key, default))
  end
