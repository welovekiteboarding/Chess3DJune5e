defmodule Symphony1.Runtime do
  ...
  alias Symphony1.Core.{Policy, QueueLauncher, QueueScheduler, RunCoordinator, Worker, Workspace}
  ...
  def run(opts \\ []) do
    with {:ok, run_attrs} <-
      ...
      if Keyword.get(opts, :once, false) do
        ...
        runtime_starter =
          Keyword.get(opts, :runtime_starter, &Symphony1.Application.start_queue_runtime/1)
        ...
        with {:ok, _apps} <- app_starter.(:symphony_1),
             {:ok, poller} <- runtime_starter.(runtime) do
          {:ok, %{run_attrs: run_attrs, poller: poller}}
        end
      end
    end
  end
  ...
  defp ensure_queue_task_supervisor! do
    case Process.whereis(Symphony1.Core.QueueTaskSupervisor) do
      nil ->
        {:ok, _pid} = Task.Supervisor.start_link(name: Symphony1.Core.QueueTaskSupervisor)
        :ok
  ...
  @spec build_run_attrs(String.t(), String.t(), keyword()) :: {:ok, map()} | {:error, term()}
  def build_run_attrs(cwd \\ File.cwd!(), graph_path \\ @graph_path, opts \\ []) do
  ...
  defp wait_for_active_runs(queue_scheduler, timeout_ms, run_attrs) do
    entries_by_ref =
      ...
      |> Map.values()
      |> Map.new(fn entry -> {entry.task.ref, entry} end)
    ...
    {results, failures} =
      ...
      |> Enum.map(fn {task, reply} ->
        entry = Map.fetch!(entries_by_ref, task.ref)

        case reply do
          {:ok, result} -> result
          {:exit, reason} -> {:error, {:task_crashed, reason}}
          nil -> shutdown_and_recover_timed_out_run(task, entry, run_attrs)
        end
      end)
      |> Enum.reduce({[], []}, fn
  ...
  defp shutdown_and_recover_timed_out_run(task, entry, run_attrs) do
    Task.shutdown(task, :brutal_kill)
    recover_timed_out_run(entry, run_attrs)
  end
