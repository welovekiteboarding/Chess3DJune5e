defmodule Symphony1.MergeRuntime do
  ...
  def run(opts \\ []) do
    ...
    with {:ok, merge_attrs} <- build_merge_attrs(cwd, opts) do
      merge_attrs = Map.put(merge_attrs, :graph_path, graph_path)
      ...
      if Keyword.get(opts, :once, false) do
        ...
        merge_runtime_starter =
          Keyword.get(
            opts,
            :merge_runtime_starter,
            &Symphony1.Application.start_merge_runtime/1
          )
        ...
        with {:ok, _apps} <- app_starter.(:symphony_1),
             {:ok, poller} <- merge_runtime_starter.(runtime) do
          {:ok, %{merge_attrs: merge_attrs, poller: poller}}
        end
      end
    end
  end

  @spec build_merge_attrs(String.t(), keyword()) :: {:ok, map()} | {:error, term()}
  def build_merge_attrs(cwd \\ File.cwd!(), opts \\ []) do
    ...
    with {:ok, intent} <- SetupIntent.load(Path.join(cwd, @setup_intent_path)) do
      ...
      with {:ok, linear_config} <- RuntimeConfig.linear_config(team_key, opts) do
        {:ok,
         %{
  ...
  defp merge_once(merge_attrs, graph_path, opts) do
    linear_requester = Keyword.get(opts, :linear_requester, &Linear.request/3)
    ...
    linear_poller =
      Keyword.get(
        ...
        :linear_poller,
        fn config ->
          Linear.poll_issue_in_states(config, ["Human Review", "Merging"], linear_requester)
        end
      )
    ...
    check_reader =
      Keyword.get(opts, :check_reader, fn pull_request ->
        GitHub.pull_request_checks(pull_request, github_runner)
      end)
        ...
        case github_resolver.(%{
          ...
          {:ok, pull_request} ->
            case merge_approval_status(approval_reader, merge_attrs.workspace, issue.identifier) do
              {:ok, :approved} ->
                with {:ok, :passing} <- merge_check_status(check_reader, pull_request),
                     :ok <-
                       record_merge_event(
                  ...
                  {:ok, {:blocked, _checks}} ->
                    {:ok,
                     %{
                       results: [],
                       merge_attrs: merge_attrs,
                       report: skipped_report(issue.identifier, "CI checks are not passing")
                     }}
              ...
              {:ok, :missing} ->
                {:ok,
                 %{
                   ...
                   merge_attrs: merge_attrs,
                   report:
                     skipped_report(issue.identifier, "no approved review artifact was found")
                 }}
  ...
  defp merge_check_status(_check_reader, %{status: :merged}), do: {:ok, :passing}
  defp merge_check_status(check_reader, pull_request), do: check_reader.(pull_request)
  ...
  defp no_work_report do
    %{
      status: :no_work,
      summary: "Merge poll found no issues in Human Review or Merging."
    }
  end
