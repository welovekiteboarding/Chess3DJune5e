defmodule Symphony1.Core.QueueScheduler do
  ...
  defp active_issue_identifiers(active_runs) do
    active_runs
    |> Map.values()
    |> Enum.reduce([], fn %{metadata: metadata}, acc ->
      maybe_add_issue_identifier(acc, metadata)
    end)
    |> Enum.reverse()
  end
  ...
  defp no_work_summary(team_key),
    do: "Queue drain found no claimable issues for team #{team_key}."
