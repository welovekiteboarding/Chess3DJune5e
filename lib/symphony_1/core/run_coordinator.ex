defmodule Symphony1.Core.RunCoordinator do
  ...
  alias Symphony1.Observability.{Artifacts, Recorder, RuntimeStatus}
  alias Symphony1.Planning.Graph
  alias Symphony1.Project.RepoAdapter

  @default_finalization_repair_attempts 1
  @timeout_recoverable_states ["In Progress", "Finalizing"]
  ...
  def run_issue(
    ...
    with {:ok, issue} <-
           Linear.poll_eligible_issue(linear_config, allowed_issue_identifiers, requester) do
      prepare_linear_run_issue(
        issue,
        attrs,
        linear_config,
        requester,
  ...
  @spec recover_claimed_issue_failure(map(), map(), String.t(), atom(), term()) ::
          {:error, term()}
  def recover_claimed_issue_failure(issue, attrs, workspace_path, stage, reason) do
    recover_failed_run(
      issue,
      Map.put(attrs, :workspace, workspace_path),
      stage,
      reason
    )
  end
  ...
  def recover_timed_out_issue(issue, %{linear_config: linear_config} = attrs) do
    requester = Map.get(attrs, :linear_requester, &Linear.request/3)

    case Linear.find_issue_by_identifier(linear_config, issue.identifier, requester) do
      {:ok, current_issue} ->
        recover_timed_out_current_issue(current_issue, attrs)
      ...
      {:error, reason} ->
        record_issue_event(
          attrs,
          issue,
          "timeout_recovery_refresh_failed",
          "recovery",
        ...
        Logger.warning(
          "symphony.coordinator: timeout recovery could not refresh #{issue.identifier}: #{inspect(reason)}"
        )
  ...
  defp recover_timed_out_current_issue(%{state: state} = issue, attrs)
       when state in @timeout_recoverable_states do
    recover_failed_run(issue, attrs, :task_timeout, :task_timeout)
  end

  defp recover_timed_out_current_issue(issue, attrs) do
    record_issue_event(
      attrs,
      issue,
      "timeout_recovery_skipped",
      "recovery",
      issue_event_details(issue, attrs,
        failure_reason: "task timed out after issue advanced to #{issue.state}"
      ),
      severity: "warning"
    )

    Logger.warning(
      "symphony.coordinator: skipping timeout recovery for #{issue.identifier}; current state is #{issue.state}"
    )

    {:error, {:task_timeout_after_issue_advanced, issue}}
  end

  @spec resume_finalizing_issue(map(), map()) :: {:ok, map()} | {:error, term()}
  def resume_finalizing_issue(run, attrs) do
    attrs = normalize_run_attrs(attrs)
    github_runner = Map.get(attrs, :github_runner, &System.cmd/3)
    repo_finalizer = Map.get(attrs, :repo_finalizer, &RepoAdapter.finalize_workspace/2)
    progress_reporter = Map.get(attrs, :progress_reporter, fn _message -> :ok end)
    ...
    record_issue_event(
      attrs,
      run.issue,
      "finalization_resume_started",
      "finalization",
      run_event_details
    )

    Logger.info("symphony.coordinator: resuming finalization for #{run.issue.identifier}")
    progress_reporter.("Resuming finalization for #{run.issue.identifier}")
  ...
  def open_review(%{workspace: workspace} = attrs, github_runner \\ &System.cmd/3) do
    ...
    with {:ok, branch} <- current_branch(workspace),
         {:ok, local_head_sha} <- current_head_sha(workspace, attrs),
         {:ok, pull_request} <-
           find_or_open_pull_request(attrs, branch, local_head_sha, title, body, github_runner) do
      {:ok,
       %{
         issue: attrs.issue,
  ...
  defp find_or_open_pull_request(attrs, branch, local_head_sha, title, body, github_runner) do
    ...
    pr_attrs = %{
      ...
      idempotency_after_pull_request_create_record:
        Map.get(attrs, :idempotency_after_pull_request_create_record),
      issue_identifier: attrs.issue.identifier,
      local_head_sha: local_head_sha,
      observability_root: observability_root(attrs),
      repo: attrs.repo,
      title: title,
      validation_commands: RepoAdapter.provenance_validation_commands(attrs)
    }
  ...
  def merge_review(attrs, github_runner \\ &System.cmd/3) do
    ...
    artifact_preserver = Map.get(attrs, :artifact_preserver, &preserve_run_artifacts/1)
    ...
    with {:ok, merging_issue, pull_request} <- merge_pull_request(attrs, github_runner),
         ...
         {:ok, _artifact_details} <-
           preserve_run_artifacts_for_done_issue(attrs, done_issue, artifact_preserver),
      ...
      record_runtime_status_for_done_issue(done_issue, attrs)
         ...
         {:ok, merged_pull_request} <-
           GitHub.merge_pull_request(
             Map.merge(pull_request, %{
               graph_task_id: graph_task_id_for_issue(issue, attrs),
               idempotency_after_pull_request_merge_record:
                 Map.get(attrs, :idempotency_after_pull_request_merge_record),
      ...
      {:ok, workspace} ->
        ...
        record_issue_event(
          ...
          issue_event_details(
            issue,
            attrs,
            [branch: branch, workspace_path: workspace] ++ workspace_base_details(workspace)
          )
        )
  ...
  defp prepare_linear_run_issue(
    ...
    build_claimed_run(issue, attrs, workspace_root, workflow_path)
  end

  defp prepare_linear_run_issue(
         %{state: "Finalizing"} = issue,
         attrs,
         _linear_config,
         _requester,
         workspace_root,
    ...
    case build_claimed_run(issue, attrs, workspace_root, workflow_path) do
      {:ok, run} -> {:ok, Map.put(run, :resume_stage, :finalizing)}
      other -> other
    end
  end
  ...
  defp prepare_linear_run_issue(
    ...
    {:error, {:unsupported_eligible_state, issue.identifier, issue.state}}
  end

  defp execute_worker(run, attrs, worker_adapter) do
    prompt = issue_prompt(run.issue, attrs)
    execute_worker_with_prompt(run, attrs, worker_adapter, prompt)
  end

  defp execute_worker_with_prompt(run, attrs, worker_adapter, prompt) do
    timeout_ms = Map.get(attrs, :worker_timeout_ms, 60_000)

    worker_opts =
      [timeout_ms: timeout_ms] ++ worker_log_opts(run, attrs)
    ...
    worker_attrs =
      ...
      |> Map.merge(Enum.into(worker_log_opts(run, attrs), %{}))

    cond do
      worker_supports_session?(worker_adapter) ->
        execute_worker_session(worker_adapter, worker_attrs, prompt, worker_opts)

      worker_supports_run_once?(worker_adapter) ->
        worker_run_once(worker_adapter, worker_attrs, prompt, worker_opts)

      true ->
        {:error, {:unsupported_worker_adapter, worker_adapter}}
    end
  end
  ...
  defp preserve_run_artifacts_for_done_issue(attrs, done_issue, artifact_preserver) do
    case artifact_preserver.(attrs) do
      {:ok, artifact_details} ->
        record_issue_event(
          attrs,
          done_issue,
          "artifacts_preserved",
          "observability",
          issue_event_details(done_issue, attrs, artifact_details)
        )
      ...
      {:error, reason} ->
        record_issue_event(
          ...
          "observability",
          issue_event_details(done_issue, attrs, failure_reason: format_failure_reason(reason)),
          severity: "warning"
        )

        {:error, {:artifact_preservation_failed, done_issue.identifier, reason}}
    end
  end
  ...
  defp classify_failure(:queue_launch_dispatch), do: "queue"
  ...
  defp workspace_base_details(workspace) do
    case Workspace.metadata(workspace) do
      {:ok, metadata} ->
        [
          base_branch: Map.get(metadata, "base_branch"),
          base_branch_sha: Map.get(metadata, "base_branch_sha"),
          base_branch_fetched: Map.get(metadata, "base_branch_fetched")
        ]
  ...
  defp worker_log_opts(run, _attrs) do
    Worker.log_paths(:worker, Artifacts.workspace_worker_dir(run.workspace))
  end

  defp preserve_run_artifacts(
         %{workspace: workspace, issue: %{identifier: issue_identifier}} = attrs
       ) do
    case observability_root(attrs) do
      nil ->
        {:ok, %{}}
      ...
      repo_root ->
        with {:ok, worker_details} <-
               Artifacts.preserve_worker_artifacts(repo_root, issue_identifier, workspace) do
          {:ok,
           Map.merge(
             worker_details,
             Artifacts.describe_review_artifacts(repo_root, issue_identifier)
           )}
        end
    end
  end
  ...
  defp record_runtime_status_for_done_issue(issue, attrs) do
    case observability_root(attrs) do
      nil ->
        :ok
      ...
      root ->
        result =
          try do
            runtime_status_checker(attrs).(root, runtime_status_opts(issue, attrs))
          rescue
            error ->
              runtime_status_exception_result(issue, attrs, error)
          end

        Recorder.record_runtime_status(
          root,
          issue.identifier,
          graph_task_id_for_issue(issue, attrs),
          result
        )
    end
  end

  defp runtime_status_checker(attrs) do
    Map.get(attrs, :runtime_status_checker, &RuntimeStatus.check/2)
  end

  defp runtime_status_exception_result(issue, attrs, error) do
    pull_request_url = get_in(attrs, [:pull_request, :url])
    error_detail = format_runtime_status_error(error)
    failed_check = "Runtime status recorder"
  ...
  defp format_runtime_status_error(%_{} = error) do
    module =
      error.__struct__
      |> Module.split()
      |> Enum.join(".")

    "#{module}: #{Exception.message(error)}"
  end

  defp format_runtime_status_error(error), do: inspect(error)

  defp runtime_status_opts(issue, attrs) do
    [issue: issue.identifier]
    |> maybe_put_runtime_status_graph(Map.get(attrs, :graph_path))
  end

  defp maybe_put_runtime_status_graph(opts, nil), do: opts
  defp maybe_put_runtime_status_graph(opts, ""), do: opts
  defp maybe_put_runtime_status_graph(opts, graph_path), do: Keyword.put(opts, :graph, graph_path)
  ...
  defp worker_supports_session?(worker_adapter) when is_map(worker_adapter) do
    Map.has_key?(worker_adapter, :start_session) and
      Map.has_key?(worker_adapter, :run_prompt) and
      Map.has_key?(worker_adapter, :stop_session)
  end

  defp worker_supports_session?(worker_adapter) do
    function_exported?(worker_adapter, :start_session, 1) and
      (function_exported?(worker_adapter, :run_prompt, 3) or
         function_exported?(worker_adapter, :run_prompt, 2)) and
      function_exported?(worker_adapter, :stop_session, 1)
  end
  ...
  defp current_head_sha(_workspace, %{finalization: %{local_head_sha: local_head_sha}})
       when is_binary(local_head_sha) and local_head_sha != "" do
    {:ok, local_head_sha}
  end

  defp current_head_sha(workspace, _attrs) do
