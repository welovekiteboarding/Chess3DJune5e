defmodule Symphony1.Core.Workspace do
  ...
  def create(%{root: root, issue_id: issue_id} = attrs) do
    with {:ok, workspace_path} <- workspace_path(root, issue_id),
         ...
         {:ok, workspace_details} <- materialize_workspace(workspace_path, attrs),
  ...
  def metadata(workspace_path) do
    with {:ok, contents} <- File.read(workspace_metadata_path(workspace_path)),
         {:ok, metadata} <- Jason.decode(contents) do
      {:ok, metadata}
    else
      {:error, reason} -> {:error, reason}
    end
  ...
  defp materialize_workspace(workspace_path, %{source_repo: source_repo, branch: branch} = attrs) do
    cond do
      File.dir?(workspace_path) ->
        with :ok <- reuse_existing_workspace(workspace_path, branch) do
          {:ok, %{base_branch_fetched: false}}
        end

      ref_exists?(source_repo, branch) ->
        with :ok <- add_existing_branch_worktree(source_repo, branch, workspace_path) do
          {:ok, %{base_branch_fetched: false}}
        end

      true ->
        with {:ok, start_point} <- workspace_start_point(source_repo, attrs),
             :ok <- add_new_branch_worktree(source_repo, branch, workspace_path, start_point.ref) do
          {:ok, Map.delete(start_point, :ref)}
        end
    end
  end
  ...
  defp materialize_workspace(workspace_path, _attrs) do
    case File.mkdir_p(workspace_path) do
      :ok -> {:ok, %{base_branch_fetched: false}}
      {:error, reason} -> {:error, reason}
    end
  end
  ...
  defp workspace_start_point(source_repo, attrs) do
    ...
    cond do
      base_branch == "" ->
        {:ok, %{ref: "HEAD", base_branch_fetched: false}}

      origin_configured?(source_repo) ->
        with :ok <- fetch_remote_branch_with_retry(source_repo, base_branch),
          ...
          {:ok,
           %{
             ...
             base_branch_sha: fetched_ref_sha(source_repo, ref),
      ...
      ref_exists?(source_repo, base_branch) ->
        {:ok, %{ref: base_branch, base_branch: base_branch, base_branch_fetched: false}}
  ...
  defp origin_configured?(source_repo) do
    case System.cmd("git", ["-C", source_repo, "remote", "get-url", "origin"],
           stderr_to_stdout: true
         ) do
      {_output, 0} -> true
  ...
  defp fetch_remote_branch_with_retry(source_repo, branch) do
    case fetch_remote_branch(source_repo, branch) do
      :ok ->
        :ok

      {:error, _reason} ->
        Process.sleep(1_000)
        fetch_remote_branch(source_repo, branch)
    end
  end
  ...
  defp fetch_remote_branch(source_repo, branch) do
    case System.cmd(
      ...
      {_output, 0} -> :ok
      {output, _exit_status} -> {:error, classify_fetch_failure(output)}
    end
  end

  defp classify_fetch_failure(output) do
    output = String.trim(output)
    downcased = String.downcase(output)

    class =
      cond do
        String.contains?(downcased, "not appear to be a git repository") or
          String.contains?(downcased, "repository not found") or
            String.contains?(downcased, "couldn't find remote ref") ->
          :remote_not_found

        String.contains?(downcased, "authentication failed") or
          String.contains?(downcased, "permission denied") or
            String.contains?(downcased, "access rights") ->
          :auth_failed

        String.contains?(downcased, "could not resolve host") or
          String.contains?(downcased, "network is unreachable") or
            String.contains?(downcased, "failed to connect") ->
          :network_unreachable

        String.contains?(downcased, "non-fast-forward") or
          String.contains?(downcased, "fetch declined") or
            String.contains?(downcased, "would clobber existing tag") ->
          :non_fast_forward
  ...
  defp build_workspace_registration(workspace_path, root, issue_id, token, workspace_details) do
    with {:ok, normalized_issue_id} <- issue_directory_name(issue_id) do
      metadata =
        ...
        |> maybe_put_metadata(:base_branch, Map.get(workspace_details, :base_branch))
        |> maybe_put_metadata(:base_branch_sha, Map.get(workspace_details, :base_branch_sha))
        |> maybe_put_metadata(
          :base_branch_fetched,
          Map.get(workspace_details, :base_branch_fetched, false)
        )
  ...
  defp write_workspace_metadata(workspace_path, metadata) do
    ...
    with :ok <- File.mkdir_p(Path.dirname(metadata_path)),
         :ok <-
           File.write(
             metadata_path,
             Jason.encode!(string_key_metadata(metadata))
           ) do
      :ok
    end
  end
  ...
  defp ref_sha(source_repo, ref) do
    case System.cmd("git", ["-C", source_repo, "rev-parse", "--verify", ref],
           stderr_to_stdout: true
         ) do
      {output, 0} -> String.trim(output)
      {_output, _exit_status} -> nil
    end
  end

  defp fetched_ref_sha(source_repo, ref) do
    case ref_sha_reader().(source_repo, ref) do
      sha when is_binary(sha) and sha != "" -> sha
      _ -> "unknown"
    end
  end

  defp ref_sha_reader do
    Application.get_env(:symphony_1, :workspace_ref_sha_reader, &ref_sha/2)
  end
