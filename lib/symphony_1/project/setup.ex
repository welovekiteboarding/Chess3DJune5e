defmodule Symphony1.Project.Setup do
  ...
  defp verified_state(intent) do
    ...
    config_blockers = config_blockers(intent)
    blockers = env_blockers ++ github_blockers ++ config_blockers
  ...
  defp config_blockers(intent) do
    case get_in(intent, ["linear", "team_key"]) do
      team_key when is_binary(team_key) and team_key != "" -> []
      _ -> ["missing_linear_team_key"]
    end
  end
  ...
  defp blocker_for(:missing_linear_team_key), do: "missing_linear_team_key"
