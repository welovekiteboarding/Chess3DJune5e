defmodule Symphony1.Project.DependencySafety do
  ...
  @npm_audit_args ["audit", "--omit=dev", "--json"]
  @npm_audit_fix_args ["audit", "fix", "--omit=dev", "--ignore-scripts"]
  ...
  defp npm_audit_fix(workspace, runner, issue_identifier) do
    case run_command(runner, "npm", @npm_audit_fix_args, workspace, issue_identifier) do
      {:ok, _output} ->
        :ok
  ...
  defp run_command(runner, command, args, workspace, issue_identifier, opts \\ []) do
    ...
    sanitized_output = EventLog.sanitize_value(trimmed_output)

    log_level = if exit_status == 0, do: :info, else: :warning

    Logger.log(
      log_level,
      "symphony.dependency_safety: finish issue=#{issue_identifier} cmd=#{inspect(command_string)} exit=#{exit_status} elapsed_ms=#{elapsed_ms} output=#{inspect(sanitized_output)}"
    )
