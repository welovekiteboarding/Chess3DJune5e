defmodule Symphony1.Planning.Graph do
  ...
  @type durability_guarantee :: :synced_tempfile_atomic_rename_with_parent_fsync_when_supported
  ...
  `:synced_tempfile_atomic_rename_with_parent_fsync_when_supported` means the
  graph contents are written to a temp file, the temp file is data-synced, it
  is atomically renamed into place, and the containing directory is synced
  after the rename when the underlying platform supports directory fsync.
  """
  @spec durability_guarantee() :: durability_guarantee()
  def durability_guarantee, do: :synced_tempfile_atomic_rename_with_parent_fsync_when_supported
  ...
  @doc """
  Parses an allowlisted task command string into `{executable, argv}`.
  ...
  """
  @spec parse_task_command(String.t()) ::
          {:ok, {String.t(), [String.t()]}}
          | {:error, {:invalid_task_command, String.t(), term()}}
  def parse_task_command(command) when is_binary(command) do
    trimmed = String.trim(command)

    cond do
      trimmed == "" ->
        {:error, {:invalid_task_command, command, :empty}}

      token = first_disallowed_shell_token(trimmed) ->
        {:error, {:invalid_task_command, command, {:shell_syntax, token}}}

      true ->
        trimmed
        |> String.split()
        |> parse_task_command_tokens(command)
    end
  end

  def parse_task_command(command),
    do: {:error, {:invalid_task_command, inspect(command), :not_a_string}}
  ...
  defp parse_task_command_tokens([], command) do
    {:error, {:invalid_task_command, command, :empty}}
  end

  defp parse_task_command_tokens(["mix", "check"], _command), do: {:ok, {"mix", ["check"]}}

  defp parse_task_command_tokens(["mix", "test" | rest], command) do
    with :ok <- validate_mix_test_args(rest, command) do
      {:ok, {"mix", ["test" | rest]}}
    end
  end

  defp parse_task_command_tokens(["test", "-f", path], command) do
    with :ok <- validate_relative_file_check_path(path, command) do
      {:ok, {"test", ["-f", path]}}
    end
  end

  defp parse_task_command_tokens(["npm", "install"], _command) do
    {:ok, {"npm", ["install"]}}
  end

  defp parse_task_command_tokens(["npm", "run", "lint"], _command) do
    {:ok, {"npm", ["run", "lint"]}}
  end
  ...
  defp parse_task_command_tokens(["npm", "run", script | _rest], command) do
    {:error, {:invalid_task_command, command, {:invalid_npm_script, script}}}
  end

  defp parse_task_command_tokens(["npm", command_name | _rest], command) do
    {:error, {:invalid_task_command, command, {:invalid_npm_command, command_name}}}
  end

  defp parse_task_command_tokens([executable | _rest], command) do
    {:error, {:invalid_task_command, command, {:unknown_command, executable}}}
  end

  defp validate_mix_test_args(rest, command) do
    case Enum.split_while(rest, &(&1 != "--seed")) do
      {paths, []} ->
        validate_mix_test_paths(paths, command)

      {paths, ["--seed", seed]} ->
        with :ok <- validate_mix_test_paths(paths, command),
             :ok <- validate_seed(seed, command) do
          :ok
        end

      {_paths, ["--seed"]} ->
        {:error, {:invalid_task_command, command, {:invalid_arguments, "missing --seed value"}}}

      {_paths, ["--seed", _seed | _extra]} ->
        {:error,
         {:invalid_task_command, command,
          {:invalid_arguments, "unexpected arguments after --seed"}}}

      {_paths, [unexpected | _extra]} ->
        {:error,
         {:invalid_task_command, command,
          {:invalid_arguments, "unexpected mix test argument: #{unexpected}"}}}
    end
  end

  defp validate_mix_test_paths(paths, command) do
    cond do
      paths == [] ->
        {:error,
         {:invalid_task_command, command,
          {:invalid_arguments, "mix test requires at least one test path"}}}

      Enum.all?(paths, &valid_mix_test_path?/1) ->
        :ok

      Enum.any?(paths, &(is_binary(&1) and String.starts_with?(&1, "-"))) ->
        {:error,
         {:invalid_task_command, command,
          {:invalid_arguments, "mix test only allows test paths before optional --seed"}}}

      true ->
        {:error,
         {:invalid_task_command, command,
          {:invalid_arguments, "mix test paths must stay under test/ and end in .exs"}}}
    end
  end

  defp valid_mix_test_path?(path) when is_binary(path) and path != "" do
    Path.type(path) == :relative and
      String.starts_with?(path, "test/") and
      String.ends_with?(path, ".exs") and
      not Enum.member?(String.split(path, "/", trim: true), "..")
  end

  defp valid_mix_test_path?(_path), do: false

  defp validate_seed(seed, command) do
    if seed =~ ~r/^\d+$/ do
      :ok
    else
      {:error, {:invalid_task_command, command, {:invalid_arguments, "seed must be an integer"}}}
    end
  end

  defp validate_relative_file_check_path(path, command) do
    cond do
      path == "" ->
        {:error, {:invalid_task_command, command, {:invalid_arguments, "missing file path"}}}

      Path.type(path) == :absolute ->
        {:error,
         {:invalid_task_command, command,
          {:invalid_arguments, "file check path must be relative"}}}

      String.split(path, "/", trim: true) |> Enum.member?("..") ->
        {:error,
         {:invalid_task_command, command,
          {:invalid_arguments, "file check path cannot traverse parent directories"}}}

      not String.starts_with?(path, "docs/") ->
        {:error,
         {:invalid_task_command, command,
          {:invalid_arguments, "file check path must stay under docs/"}}}
  ...
  defp first_disallowed_shell_token(command) do
    Enum.find_value(@disallowed_shell_tokens, fn token ->
      if String.contains?(command, token), do: token, else: nil
    end) ||
      Enum.find_value(@disallowed_shell_chars, fn token ->
        if String.contains?(command, token), do: token, else: nil
      end)
  end
      ...
      {:ok, device} ->
        result =
          with :ok <- wrap_atomic_stage(:write, file_ops.file_write(device, contents)),
               ...
               :ok <- wrap_atomic_stage(:rename, file_ops.rename(temp_path, path)),
               :ok <- atomic_sync_parent_dir(file_ops, path) do
            :ok
          end
  ...
  defp cleanup_atomic_write_result(
         {:error, {:atomic_write_failed, :parent_directory_sync, _reason}} = error,
         _file_ops,
         _device,
         _temp_path
       ),
  ...
  defp atomic_sync_parent_dir(file_ops, path) do
    result =
      if function_exported?(file_ops, :sync_parent_dir, 1) do
        file_ops.sync_parent_dir(path)
      else
        :ok
      end
    ...
    case result do
      ...
      {:error, reason} ->
        if unsupported_parent_directory_sync_reason?(reason) do
          :ok
        else
          {:error, {:atomic_write_failed, :parent_directory_sync, reason}}
        end

      other ->
        {:error, {:atomic_write_failed, :parent_directory_sync, other}}
    end
  end

  defp unsupported_parent_directory_sync_reason?(reason),
    do: reason in @unsupported_parent_directory_sync_reasons
  ...
  @doc false
  def sync_parent_dir(path) do
    parent_dir = path |> Path.dirname() |> String.to_charlist()

    case :file.open(parent_dir, [:read, :directory, :raw]) do
      {:ok, device} ->
        try do
          :file.sync(device)
        after
