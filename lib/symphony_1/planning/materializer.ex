defmodule Symphony1.Planning.Materializer do
    ...
    case maybe_block_for_recovery_snapshot(graph, valid_to_create, opts) do
      :ok ->
        with {:ok, graph, materialized} <-
               maybe_create_issues(valid_to_create, graph, linear_config, issue_creator, dry_run?) do
          {:ok,
           %{
             graph: graph,
  ...
  * `persistence_error_result/0` when Linear side effects succeeded but graph persistence failed
  ...
  defp maybe_create_issues(tasks, graph, _config, _issue_creator, true) do
    {:ok, graph, Enum.map(tasks, &preview_materialized_entry/1)}
  end

  defp maybe_create_issues(tasks, graph, config, issue_creator, false) do
    create_issues(tasks, graph, config, issue_creator)
  end

  defp preview_materialized_entry(task) do
    %{
      task_id: task.id,
      linear_issue_id: "__dry_run__",
      linear_issue_identifier: "__dry_run__"
    }
  end
