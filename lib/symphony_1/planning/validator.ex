defmodule Symphony1.Planning.Validator do
  ...
  def validate_task_admission(%Graph.Task{} = task, opts \\ []) do
    ...
    with :ok <- Graph.validate_task_admission(task),
         :ok <- validate_product_task(task, project_type),
         :ok <- validate_task_commands(task),
         :ok <- validate_code_task_mix_check(task) do
      :ok
    end
  end
  ...
  defp validate_code_task_mix_check(%Graph.Task{kind: "code"} = task) do
    if includes_mix_check?(task) do
      :ok
    else
      {:error, {:admission_failed, task.id, "code tasks must include mix check"}}
    end
  end

  defp validate_code_task_mix_check(_task), do: :ok

  defp includes_mix_check?(%Graph.Task{
         validation: %Graph.Validation{commands: commands}
       })
       when is_list(commands) do
    Enum.any?(commands, &(is_binary(&1) and String.trim(&1) == "mix check"))
  end

  defp includes_mix_check?(_task), do: false

  defp validate_task_commands(%Graph.Task{
         id: task_id,
         validation: %Graph.Validation{} = validation
       }) do
    with :ok <-
           validate_command_list(task_id, validation.setup_commands, "validation.setup_commands"),
         :ok <- validate_command_list(task_id, validation.commands, "validation.commands") do
      :ok
    end
  end

  defp validate_task_commands(_task), do: :ok

  defp validate_command_list(task_id, commands, field_name) when is_list(commands) do
    Enum.reduce_while(commands, :ok, fn command, :ok ->
      case Graph.parse_task_command(command) do
        {:ok, _parsed} ->
          {:cont, :ok}

        {:error, {:invalid_task_command, command_text, reason}} ->
          {:halt,
           {:error,
            {:admission_failed, task_id,
             "#{field_name} contains unsupported command: #{command_text} (#{format_command_error(reason)})"}}}
      end
    end)
  end

  defp validate_command_list(_task_id, _commands, _field_name), do: :ok

  defp format_command_error(:empty), do: "empty command"
  defp format_command_error(:not_a_string), do: "command must be a string"
  defp format_command_error({:shell_syntax, token}), do: "shell syntax: #{token}"
  defp format_command_error({:unknown_command, command}), do: "unknown command: #{command}"

  defp format_command_error({:invalid_npm_command, command}),
    do: "invalid npm command: #{command}"

  defp format_command_error({:invalid_npm_script, script}), do: "invalid npm script: #{script}"
  defp format_command_error({:invalid_arguments, message}), do: message
