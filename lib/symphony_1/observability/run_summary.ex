defmodule Symphony1.Observability.RunSummary do
  ...
  alias Symphony1.Observability.Artifacts
  ...
  @type events_report :: %{
          ...
          summary: String.t(),
          artifact_status: Artifacts.issue_artifact_status() | nil,
          events: [map()],
  ...
  def events_report(cwd, opts \\ []) when is_binary(cwd) and is_list(opts) do
    ...
    artifact_status = artifact_status(cwd, issue_identifier)
  ...
  def render_events_report(report) when is_map(report) do
    [
      ...
      report.summary,
      format_artifact_section(report),
      "",
      "Recent events:",
  ...
  @spec record_issue_event(String.t(), map(), keyword()) :: :ok | {:error, term()}
  def record_issue_event(cwd, entry, opts \\ [])

  def record_issue_event(cwd, entry, opts)
      when is_binary(cwd) and is_map(entry) and is_list(opts) do
    ...
    case issue_identifier(entry) do
      ...
      issue_identifier ->
        EventLog.with_lock(
          lock_path(cwd, issue_identifier),
          fn -> write_summary(cwd, issue_identifier, entry) end,
          opts
        )
    end
  end

  defp write_summary(cwd, issue_identifier, entry) do
    ...
    with :ok <- File.mkdir_p(Path.dirname(summary_path)),
      ...
      {:error, _reason} = error -> {:error, {:run_summary_write_failed, error}}
      error -> {:error, {:run_summary_write_failed, error}}
    end
  end
  ...
  defp lock_path(cwd, issue_identifier) do
  ...
  defp artifact_status(_cwd, nil), do: nil

  defp artifact_status(cwd, issue_identifier) do
    Artifacts.issue_artifact_status(cwd, issue_identifier)
  end
  ...
  defp format_artifact_section(%{artifact_status: nil}), do: []

  defp format_artifact_section(%{artifact_status: artifact_status}) do
    [
      "",
      "Recorder artifacts:",
      format_phase_status("worker", artifact_status.worker, "worker.jsonl"),
      format_phase_status("review", artifact_status.review, "review.jsonl"),
      format_codex_exec_runtime_command_status(artifact_status.codex_exec_runtime_command),
      format_codex_exec_text_mention_status(artifact_status.codex_exec_text_mentions)
    ]
  end

  defp format_phase_status(label, %{present?: false}, _jsonl_name) do
    "#{label} artifacts: absent"
  end

  defp format_phase_status(label, status, jsonl_name) do
    lines =
      case status.jsonl_lines do
        value when is_integer(value) -> Integer.to_string(value)
        _value -> "missing"
      end

    files =
      case status.files do
        [] -> "none"
        values -> Enum.join(values, ", ")
      end

    "#{label} artifacts: present | runtime: #{fallback(status.runtime)} | " <>
      "command: #{fallback(status.command)} | #{jsonl_name} lines: #{lines} | " <>
      "path: #{status.path} | files: #{files}"
  end

  defp format_codex_exec_runtime_command_status(%{present?: false}) do
    "codex exec runtime command evidence: no"
  end

  defp format_codex_exec_runtime_command_status(%{present?: true, paths: paths}) do
    "codex exec runtime command evidence: yes | paths: #{Enum.join(paths, ", ")}"
  end

  defp format_codex_exec_text_mention_status(%{present?: false}) do
    "codex exec text mention evidence: no"
  end

  defp format_codex_exec_text_mention_status(%{present?: true, paths: paths}) do
    "codex exec text mention evidence: yes | paths: #{Enum.join(paths, ", ")}"
  end
